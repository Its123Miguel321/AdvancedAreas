...
use Its123Miguel321\AdvancedAreas\forms\EditAreaForm;
...
class EditAreaSettingsMenu{
	...
	public static function setMenuItems(Player $player, Inventory $menu) : void{
		...
		for($i = 0; $i < 54; $i++){
			...
			if(isset(self::$slots[$i])){
				$item->setLore([
					'',
					TF::GRAY . 'Status: ' . (self::$events[$player->getXuid()][self::$slots[$i]] ? TF::GREEN . 'ENABLED' : TF::RED . 'DISABLED')
				]);
	...
	public static function menu(Player $player, Area $area) : void{
		$menu = InvMenu::create(InvMenu::TYPE_DOUBLE_CHEST);
		$menu->setName($area->getDisplayName() . ' Settings');
		$menu->setListener(function(InvMenuTransaction $transaction) use($area) : InvMenuTransactionResult
		{
			return self::listener($area, $transaction);
		});

		self::$events[$player->getXuid()] = $area->getEvents();
	...
	public static function listener(Area $area, InvMenuTransaction $transaction) : InvMenuTransactionResult{
		...
		if(isset(self::$slots[$slot])){
			...
			self::$events[$player->getXuid()][$setting] = !(self::$events[$player->getXuid()][$setting]);
		...
		if($slot === 45){
			...
			return $transaction->discard()->then(function() use($player, $area){
				EditAreaForm::form($player, $area);
			});
		}elseif($slot === 49){
			$player->removeCurrentWindow();
		}elseif($slot === 53){
			...
			return $transaction->discard()->then(function() use($player, $area){
				$area->setEvents(self::$events[$player->getXuid()]);

				EditAreaForm::form($player, $area, TF::GREEN . 'Updated Settings!' . "\n\n");
			});
		}
